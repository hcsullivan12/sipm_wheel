 ...
 * main.cxx
 *
 * Copyright 2018 Hunter Sullivan
 *
 *
 */
...
int main(int argc, char **argv)
{
  ...
  if (argc < 2)
  {
    std::cerr << "Usage: " << argv[0] << " pathToConfigFile " << std::endl;
    exit(1);
  }
  ...
  wheel::HitCandidateVec hitCandidateVec;
  wheel::FileReader fr;
  fr.ReadFiles(hitCandidateVec, sipmToFilesMap, myConfig);

  // Option to output graphs
  if (myConfig.saveGraphs) SaveGraphs(fr.GetGraphs(), fr.GetMarkers(), myConfig);

  // Charaterize the sipms
  /*CharaterizeSiPMs();*/
...
void SaveGraphs(std::vector<TGraph>& graphs, std::vector<std::vector<TMarker>>& markers, const wheel::Configuration& config)
{
  ...
  TFile *f = new TFile(config.outputPath.c_str(), "RECREATE");
